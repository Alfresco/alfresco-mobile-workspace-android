                ...
                ComponentType.TEXT.value -> {
                    ...
                    binding.chip.text =
                        if (dataObj.selectedName.isNotEmpty()) {
                            ...
                            context.getLocalizedName(dataObj.category?.name?.wrapWithLimit(context, CHIP_TEXT_DISPLAY_LIMIT) ?: "")
                        }
                }
                ComponentType.FACETS.value -> {
                    if (dataObj.selectedName.isNotEmpty()) {
                        binding.chip.text = dataObj.selectedName.wrapWithLimit(context, CHIP_TEXT_DISPLAY_LIMIT, ",")
                    } else {
                        ...
                        if (localizedName == replacedString) {
                            binding.chip.text = dataObj.facets?.label?.wrapWithLimit(context, CHIP_TEXT_DISPLAY_LIMIT) ?: ""
                        } else {
                            binding.chip.text = localizedName.wrapWithLimit(context, CHIP_TEXT_DISPLAY_LIMIT)
                        }
                    }
                }
                else -> {
