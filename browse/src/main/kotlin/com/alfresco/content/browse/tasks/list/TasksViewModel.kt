...
class TasksViewModel(
            ...
            viewModelScope.launch {
                ...
                repository::getTasks.asFlow(
                    if (!isWorkflowTask) {
                        ...
                        TaskProcessFiltersPayload.defaultTasksOfProcess(state.processEntry?.id)
                    },
                ).execute {
                    when (it) {
                        is Loading -> copy(request = Loading())
     ...
     */
    fun updateChipFilterResult(
        state: TasksViewState,
        model: TaskFilterData,
        metaData: ComponentMetaData,
    ): MutableList<TaskFilterData> {
        val list = mutableListOf<TaskFilterData>()
    ...
    fun execute(
        ...
        action: Action,
    ) = action.execute(requireContext, GlobalScope)
