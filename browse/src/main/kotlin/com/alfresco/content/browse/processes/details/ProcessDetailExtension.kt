...
internal fun ProcessDetailFragment.setListeners() {
    ...
    binding.iconAssignedEdit.setSafeOnClickListener {
        withState(viewModel) { state ->
            ...
            viewLifecycleOwner.lifecycleScope.launch {
                val result =
                    showSearchUserGroupComponentDialog(
                        requireContext(),
                        state.parent,
                    )
                if (result != null) {
...
internal fun ProcessDetailFragment.setData(state: ProcessDetailViewState) {
    ...
    binding.tvAssignedValue.apply {
        text =
            if (dataEntry?.startedBy?.groupName?.isEmpty() == true && viewModel.getAPSUser().id == dataEntry.startedBy?.id) {
                requireContext().getLocalizedName(
                    dataEntry.startedBy?.let { UserGroupDetails.with(it).name } ?: getString(R.string.text_select_assignee),
                )
                ...
                requireContext().getLocalizedName(dataEntry?.startedBy?.name ?: getString(R.string.text_select_assignee))
            }
    }
    ...
    if (dataEntry?.processDefinitionId.isNullOrEmpty()) {
        ...
        binding.tvDueDateValue.text =
            if (dataEntry?.formattedDueDate.isNullOrEmpty()) {
                requireContext().getString(
                    R.string.empty_no_due_date,
                )
            } else {
                dataEntry?.formattedDueDate?.getFormattedDate(DATE_FORMAT_1, DATE_FORMAT_4)
            }
...
internal fun ProcessDetailFragment.showTitleDescriptionComponent() =
    withState(viewModel) {
        viewLifecycleOwner.lifecycleScope.launch {
            showComponentSheetDialog(
                requireContext(),
...
internal fun executeContinuation(
