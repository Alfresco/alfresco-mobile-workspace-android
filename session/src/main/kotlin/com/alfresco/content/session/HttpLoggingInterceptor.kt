...
class HttpLoggingInterceptor @JvmOverloads constructor(
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        if (logHeaders) {
            ...
            } else if (bodyHasBinaryParts(requestBody)) {
                logger.log("--> END ${request.method} (binary ${requestBody.contentLength()}-byte multipart body omitted)")
            } else {
                val buffer = Buffer()
    ...
    private fun bodyHasBinaryParts(body: RequestBody): Boolean {
        if (body is MultipartBody) {
            for (part in body.parts) {
                val transferEncoding = part.headers?.get("Content-Transfer-Encoding")
                if (transferEncoding.equals("binary")) return true
            }
        }
        return false
    }
